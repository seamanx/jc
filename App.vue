  ...
  <div style="height:100%;">
    ...
    <view-box v-ref:view-box>
      ...
      <div class="vux-demo-header-box" slot="header">
        <x-header :right-options="{showMore: true}" @on-click-more="showMenus = true" :left-options="leftOptions" :transition="headerTransition"
          :title="title" @on-click-title="scrollTop"></x-header>
      </div>
      <!--default slot-->
      ...
      <tabbar class="vux-demo-tabbar" icon-class="vux-center" v-show="!isTabbarDemo" slot="bottom">
        ...
        <tabbar-item v-link="{path:'/test'}" :selected="isDemo" badge="9">
          <span class="demo-icon-22" slot="icon">&#xe633;</span>
          <span slot="label"><span v-if="componentName" class="vux-demo-tabbar-component">{{componentName}}</span><span v-else>测试</span></span>
        </tabbar-item>
        <tabbar-item v-link="{path:'/profile'}" :selected="route.path === '/project/donate'" show-dot>
          <span class="demo-icon-22" slot="icon">&#xe630;</span>
...
import { Tabbar, TabbarItem, Loading, ViewBox, XHeader,Actionsheet } from './components-vux'
...
export default {
  ...
  computed: {
    ...
    title () {
      if (this.route.path === '/') return '爱·囧城'
